...
规则：1. 只能使用请求里给出的用户 id；2. 最多 6 步；3. 不替受邀人接受；4. 缺少会阻塞执行的负责人、明确时间或关键边界时 status=needs_input，并只问一个问题；5. 信息足够时 status=ready；6. 不捏造地址、日期、费用或医疗决定。`
...
async function runMiniMax(config: ServerConfig, request: AgentTurnRequest): Promise<AgentTurnResponse> {
  ...
  try {
    ...
    const result = await fetch(`${config.baseUrl.replace(/\/$/, '')}/chat/completions`, {
      ...
      body: JSON.stringify({ model: config.model, messages: [{ role: 'system', content: SYSTEM_PROMPT }, { role: 'user', content: context }], temperature: 1, top_p: 0.95, max_tokens: 2_000 }),
    ...
    return { ...parsed, engine: 'minimax', notice: `由 ${config.model} 生成；发布前仍需人工确认。` }
  } finally {
    clearTimeout(timer)
  }
}
