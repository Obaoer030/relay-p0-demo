...
const acceptanceField = /接受|同意|是否方便|确认.*(?:委托|邀请|负责)|能否(?:处理|负责|帮忙)/
const dateField = /日期|哪一天/
...
export function guardAgentTurn(response: AgentTurnResponse, request: AgentTurnRequest): AgentTurnResponse {
  ...
  const isCatVisit = /布丁|猫|复诊|宠物/.test(combined)
  ...
  if (isCatVisit && !hasSpecificTime) {
    return {
      ...
      draft: { ...response.draft, steps: canonicalSteps, missingFields: ['预约时间'] },
    }
  }
  ...
  if (response.status === 'needs_input' && missingFields.length > 0 && missingFields.every(isNonBlocking)) {
    ...
    return {
      ...
      question: undefined,
      draft: { ...response.draft, steps: canonicalSteps, missingFields: [] },
    }
  }
  ...
  return {
    ...response,
    question: response.status === 'needs_input' ? (firstQuestion ? `${firstQuestion}？` : firstMissingField ? `请补充${firstMissingField}。` : undefined) : undefined,
    draft: {
      ...response.draft,
