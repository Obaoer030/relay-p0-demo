...
const actionVerb = /准备|预约|预订|订购|确认|核对|收集|整理|分类|打印|提交|填写|联系|通知|发送|同步|挂号|检查|复诊|看病|就诊|取药|领取|购买|买|安装|验收|交接|归还|接送|送到|带到|办理|缴费|支付|拿到|取得|获取|完成|拍摄|拍照|拍下|报修|询价|比较|筛选|选择|等待|等候|等(?=师傅|对方|人)|测试|试运行|记录|录入|保存|查询|查看|盘点|列出|打包|装车|更新|修正|构建/
const conditionalBoundary = /^(?:如果|若|如遇|遇到|一旦|除非|当).*(?:联系|确认|询问)/
const sequenceBoundary = /(?:[。；;\n]+|，?(?:然后|接着|随后|最后|之后还需要|之后再|之后|以后还需要|以后再|再去|再把|再将)|，?再(?=准备|预约|预订|订购|确认|核对|收集|整理|打印|提交|填写|联系|通知|发送|同步|挂号|检查|复诊|看病|就诊|取药|领取|购买|安装|验收|交接|归还|办理|测试|记录|录入|保存|查询|查看|盘点|打包|更新|修正|构建)|，并(?=把|将|去|完成|联系|发送|同步|整理|购买|领取))/
...
function normalizedClauses(input: string) {
  const normalized = input
    ...
    .replace(/，(?=[^，。；;]{0,16}(?:前|后)[^，。；;]{0,6}(?:核对|检查|测试|记录|整理|联系|购买|领取|保存|查询|更新|归还|提交|构建))/g, '；')
