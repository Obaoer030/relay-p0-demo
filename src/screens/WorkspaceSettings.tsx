  ...
  return (
    <main className="workspace-page">
      ...
      <div className="workspace-settings-grid">
        <section className="workspace-panel workspace-settings-card"><Database size={22} /><div><h2>多角色演示数据</h2><p>{state.users.length} 个可切换视角、{state.matters.length} 个共享事项、{state.activity.length} 条活动保存在 localStorage。</p></div><button className="workspace-secondary-action" type="button" onClick={reset}><RotateCcw size={17} /> 恢复演示数据</button></section>
        <section className="workspace-panel workspace-settings-card"><Smartphone size={22} /><div><h2>减少动态效果</h2><p>关闭负责人圆点的移动动画，但保留人物、实心点和状态文字。</p></div><label className="workspace-switch"><input type="checkbox" checked={state.reduceMotion} onChange={(event) => dispatch({ type: 'set-reduce-motion', value: event.target.checked })} /><span />{state.reduceMotion ? '已开启' : '未开启'}</label></section>
        ...
        <section className="workspace-panel workspace-settings-card"><ExternalLink size={22} /><div><h2>完整交互演示</h2><p>演示入口使用当前工作台和同一份事项数据，不再维护独立路演模型。</p></div><Link className="workspace-secondary-action" to="/demo">进入完整演示 <ExternalLink size={16} /></Link></section>
      </div>
    </main>
  )
}
