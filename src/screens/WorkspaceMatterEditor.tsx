  ...
  return (
    <main className="workspace-page workspace-editor-page">
      ...
      <form className="workspace-form" onSubmit={submit}>
        ...
        <section className="workspace-form-section workspace-form-grid"><h2>分类与负责人</h2><label><span>日期</span><input type="date" value={dueDate} onChange={(event) => setDueDate(event.target.value)} /></label><label><span>分类</span><select value={category} onChange={(event) => setCategory(event.target.value)}>{['生活','宠物','住房','家人','伴侣','搬家','行政'].map((item) => <option key={item}>{item}</option>)}</select></label><label><span>优先级</span><select value={priority} onChange={(event) => setPriority(event.target.value as WorkspacePriority)}><option value="low">低</option><option value="normal">普通</option><option value="high">高</option></select></label><label><span>当前状态</span><select value={status} onChange={(event) => setStatus(event.target.value as WorkspaceMatterStatus)}><option value="mine">待我处理</option><option value="waiting">等待回复</option><option value="relayed">对方处理中</option><option value="completed">已完成</option></select></label><label><span>当前负责人</span><select value={ownerName} onChange={(event) => setOwnerName(event.target.value)}><option>林然</option>{state.people.map((person) => <option key={person.id}>{person.name}</option>)}</select></label></section>
